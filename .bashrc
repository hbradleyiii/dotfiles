...
PROMPT_COMMAND='if [[ "$?" != "127" ]] ; then history -a ~/.bash_history && history -n ; fi;'
...
alias rm='rm -i'
alias save-gui='pkill -HUP gnome-shell'
...
alias ssu='sudo HOME="$HOME" SSH_AUTH_SOCK="$SSH_AUTH_SOCK" SSH_CLIENT="$SSH_CLIENT" SSH_CONNECTION="$SSH_CONNECTION" bash --rcfile $HOME/.bash_profile'
...
function gitfetch() {
    ...
    local next_fetch=$(($last_fetch + 9000))
